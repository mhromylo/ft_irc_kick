...
void Server::Invite(std::string &cmd, int &fd)
{
	...
	if(scmd.size() < 3 || scmd[2].empty())// ERR_NEEDMOREPARAMS (461) if there are not enough parameters
		{senderror(461, GetClient(fd)->GetNickName(), fd, " :Not enough parameters\r\n"); return;}
	std::string channelname = scmd[2].substr(1);
	Channel *channel = GetChannel(channelname);
	if(scmd[2][0] != '#' || !channel)// ERR_NOSUCHCHANNEL (403) if the given channel does not exist
	...
	std::string rep2 = ":"+ GetChannel(channelname)->get_admin(fd)->GetNickName() + " INVITE you " + clt->GetNickName() + " " + scmd[2]+"\r\n";
